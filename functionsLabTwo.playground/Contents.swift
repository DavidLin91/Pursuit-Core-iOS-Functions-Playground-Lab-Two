...
func average(of anArray: [Double]) -> Double {
    var total = 0.0
    for num in anArray {
        total += num / Double(anArray.count)
    }
    return total
}
...
func frequencyDictionary( of stringInput: String) -> [Character: Int] {
    var charMap: [Character: Int] = [:]
    for char in stringInput {
        charMap[char] = 0
    }
    for char in stringInput {
        if var charMap2 = charMap[char] {
            charMap2 += 1
            charMap[char] = charMap2
        }
    }
    return charMap
}
...
func fizzBizz(upto inputNum: Int)-> [String] {
    var emptyArray = [String]()
    for num in 1...inputNum {
        if num % 3 == 0 {
            emptyArray.append("Fizz")
        } else if num % 5 == 0 {
            emptyArray.append("Bizz")
        } else if num % 3 == 0 && num % 5 == 0 {
            emptyArray.append("FizzBizz")
        }
        else {
